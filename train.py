...
class Network:
    ...
    def __init__(self, train_dataset, dir_, batch_size=4, testdata=[0], train_old_net=False, epoch=0):
    ...
    def create_network(self):
        self.net = ms.MySequential(nstack=5)
        self.net.hybridize(static_alloc=True, static_shape=True)
        self.net.initialize(init=mx.init.Xavier(), ctx=gpu(0))

    def load_network(self, epoch):
        self.net = ms.MySequential(nstack=5)
        self.net.load_parameters(self.dir + f"Network_export\\Epoch-{epoch}.params", ctx=gpu(0))
        # with warnings.catch_warnings():
        #     warnings.simplefilter("ignore")
        #     self.net = nn.SymbolBlock.imports(self.dir + "Network_export\\Epoch-symbol.json", ['data'],
        #                                       self.dir + f"Network_export\\Epoch-{epoch}.params", ctx=gpu(0))
        self.net.hybridize(static_alloc=True, static_shape=True)

    def create_trainer(self):
        schedule = mx.lr_scheduler.MultiFactorScheduler(step=[250, 500, 750, 1000], factor=0.5)
        schedule.base_lr = 1E-2
