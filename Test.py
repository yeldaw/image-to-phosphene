...
root_dir = f"F:\\Thesis Datasets\\mpii\\mpii_human_pose_v1\\Final\\"
image_size = 256
...
def test(net):
    for f in os.listdir(f"{root_dir}Final_test\\"):
        transformer = transforms.Compose([transforms.ToTensor()])
        c = plt.imread(f"{root_dir}Final_test\\{f}")
...
with warnings.catch_warnings():
    ...
    deserialized_net = nn.SymbolBlock.imports(root_dir + "Final_export\\Final_joint-symbol.json", ['data'],
                                              root_dir + "Final_export\\Final_joint-2500.params", ctx=gpu(0))
